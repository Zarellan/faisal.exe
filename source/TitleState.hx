...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		ngSpr2 = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('num0'));
		add(ngSpr2);
		ngSpr2.visible = false;
		ngSpr2.setGraphicSize(Std.int(ngSpr.width * 0.8));
		ngSpr2.updateHitbox();
		ngSpr2.screenCenter(X);
		ngSpr2.antialiasing = ClientPrefs.globalAntialiasing;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 8:
					deleteCoolText();
					Lib.application.window.title = "Faisal.exe";
					Lib.application.window.setIcon(Image.fromBitmapData(Paths.image("faisas").bitmap));
					createCoolText(['YOUR ARE ALREADY'], -40);
					addMoreText('DEAD', -40);
					FlxG.camera.shake(0.04, 0.60);
					ngSpr.visible = false;
					ngSpr2.visible = false;
				// credTextShit.text += '\nNewgrounds';
				...
				// credTextShit.screenCenter();
				case 10:
				FlxG.sound.playMusic(Paths.music('faisal.exe start'), 0);
					MusicBeatState.switchState(new MainMenuState());
				// credTextShit.visible = true;
				case 12:
